...
def gather_data():
    ...
    try:
        ...
        logger.info(
            f"Gathering data for {symbol} ({market_type}) from {start_date} to {end_date}"
        )
        ...
        if df.empty:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "No data could be retrieved. Check the symbol, date range, and timeframe.",
                    }
                ),
                400,
            )
...
def data_status():
    ...
    try:
        ...
        if not symbol:
            return (
                jsonify({"success": False, "error": "Symbol parameter required"}),
                400,
            )
...
def data_preview():
    ...
    try:
        ...
        if df.empty:
            return (
                jsonify({"success": False, "error": f"No data available for {symbol}"}),
                400,
            )
