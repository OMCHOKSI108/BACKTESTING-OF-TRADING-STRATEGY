...
# Import advanced crypto/forex service
from .crypto_forex_data_service import advanced_crypto_forex_service
...
class EnhancedDataService:
    def __init__(self, max_workers=10, cache_size=1000):
        ...
        self.cache_db_path = os.path.join(
            os.path.dirname(__file__), "..", "data", "cache.db"
        )
    ...
    def _cache_data(
        ...
        try:
            ...
            with sqlite3.connect(self.cache_db_path) as conn:
                ...
                conn.commit()
                logger.info(
                    f"Cached data for {symbol} (expires in {expire_hours} hours)"
                )
        except Exception as e:
    ...
    def _rate_limit_check(self, api_name: str, requests_per_minute: int = 60) -> bool:
        ...
        self.rate_limiter[api_name] = [
            req_time
            for req_time in self.rate_limiter[api_name]
    ...
    def fetch_yahoo_data(self, symbol, start_date, end_date, timeframe="1d"):
        ...
        try:
            ...
            if not self._rate_limit_check("yahoo", requests_per_minute=200):
                logger.warning(
                    f"Rate limit exceeded for Yahoo Finance, queuing request for {symbol}"
                )
                time.sleep(0.5)
    ...
    def fetch_multiple_symbols_concurrent(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(future_to_symbol):
                ...
                results[symbol] = data
                logger.info(
                    f"Completed fetching data for {symbol} ({len(data)} records)"
                )
    ...
    def fetch_finnhub_data(self, symbol, start_date, end_date, timeframe="1d"):
        ...
        try:
            ...
            if not api_key:
                logger.warning(
                    "Finnhub API key not available, skipping Finnhub data fetch"
                )
                return pd.DataFrame()

            logger.info(
                f"Fetching Finnhub data for {symbol} from {start_date} to {end_date}"
            )
            ...
            if data.get("s") != "ok":
                logger.warning(
                    f"Finnhub API returned status: {data.get('s', 'unknown')}"
                )
                return pd.DataFrame()
            ...
            logger.info(
                f"Successfully fetched {len(df)} candles from Finnhub for {symbol}"
            )
            return df
    ...
    def fetch_alphavantage_data(self, symbol, start_date, end_date, timeframe="1d"):
        ...
        try:
            ...
            if not api_key:
                logger.warning(
                    "Alpha Vantage API key not available, skipping Alpha Vantage data fetch"
                )
                return pd.DataFrame()

            logger.info(
                f"Fetching Alpha Vantage data for {symbol} from {start_date} to {end_date}"
            )

            # Alpha Vantage free tier limitations
            if timeframe in ["1m", "5m", "15m", "30m"]:
                logger.warning(
                    "Alpha Vantage intraday data requires premium subscription"
                )
                return pd.DataFrame()
            ...
            logger.info(
                f"Successfully fetched {len(df)} candles from Alpha Vantage for {symbol}"
            )
            return df
    ...
    def get_market_data(
        self, symbol, market_type, start_date, end_date, timeframe="1d"
    ):
        """
        ...
        """
        logger.info(
            f"Fetching data for {symbol} ({market_type}) from {start_date} to {end_date}"
        )
        ...
        if market_type == "Forex":
            finnhub_data = self.fetch_finnhub_data(
                symbol, start_date, end_date, timeframe
            )
        ...
        if len(all_data) == 1:
            ...
            logger.info(
                f"Using single data source: shape={combined_df.shape}, columns={list(combined_df.columns)}"
            )
        else:
            logger.info(f"Combining data from {len(all_data)} sources")
            for i, df in enumerate(all_data):
                logger.info(
                    f"DataFrame {i}: shape={df.shape}, columns={list(df.columns)}"
                )
            combined_df = pd.concat(all_data, ignore_index=True)
            logger.info(
                f"Combined DataFrame shape: {combined_df.shape}, columns: {list(combined_df.columns)}"
            )
    ...
    def clean_data(self, df):
        ...
        if final_rows < initial_rows:
            logger.info(
                f"Data cleaning removed {initial_rows - final_rows} invalid rows"
            )
    ...
    def gather_data(self, symbol, market_type, start_date, end_date, timeframe="1d"):
        """
        Enhanced main method to gather data according to user input
        Now includes advanced crypto and forex data gathering
        Saves raw and processed data automatically
        """
        logger.info(f"Gathering data for {symbol} ({market_type}) - {timeframe}")

        # For forex data, skip cache and existing processed data to ensure Yahoo Finance usage
        skip_existing_data = market_type.lower() in ["forex", "fx"]

        if not skip_existing_data:
            # Try to load existing processed data first (for stocks and crypto)
            existing_data = self.load_processed_data(symbol, timeframe)
            if not existing_data.empty:
                logger.info(f"Using existing processed data for {symbol}")
                return existing_data

        # Use advanced crypto/forex service for these market types
        if market_type.lower() in ["crypto", "cryptocurrency", "forex", "fx"]:
            try:
                logger.info(f"Using advanced {market_type} data service for {symbol}")
                ...
                try:
                    df = loop.run_until_complete(
                        advanced_crypto_forex_service.gather_advanced_data(
                            symbol, market_type, start_date, end_date, timeframe
                        )
                    )
                finally:
                    loop.close()
                ...
                if not df.empty:
                    # Remove quality_score column before saving if it exists
                    df_to_save = df.drop(columns=["quality_score"], errors="ignore")

                    # Save raw and processed data
                    self.save_raw_data(df_to_save, symbol, timeframe)
                    self.save_processed_data(df_to_save, symbol, timeframe)

                    logger.info(
                        f"Successfully gathered {len(df)} records using advanced service"
                    )
                    return df_to_save
                else:
                    logger.warning(f"Advanced service returned empty data for {symbol}")

            except Exception as e:
                logger.error(f"Error using advanced {market_type} service: {e}")
                # Fall back to traditional method
                logger.info("Falling back to traditional data gathering methods")

        # Traditional data gathering for stocks and fallback
    ...
    def batch_process_symbols(self, symbol_requests: List[Dict]) -> Dict[str, Dict]:
        ...
        """
        logger.info(
            f"Starting batch processing for {len(symbol_requests)} symbol requests"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(future_to_request):
                ...
                logger.info(
                    f"{status} {symbol}: {result.get('message', result.get('error', 'Unknown'))}"
                )

        success_count = sum(1 for r in results.values() if r["success"])
        logger.info(
            f"Batch processing completed: {success_count}/{len(symbol_requests)} successful"
        )
    ...
    def get_cache_stats(self) -> Dict:
        ...
        try:
            with sqlite3.connect(self.cache_db_path) as conn:
                ...
                # Get cache size
                cache_size_mb = (
                    os.path.getsize(self.cache_db_path) / (1024 * 1024)
    ...
    def get_multiple_stocks_data(
        ...
        def fetch_single_symbol(symbol):
            try:
                ...
                data = self.fetch_data(
                    "US Stocks", symbol, start_date_str, end_date_str, timeframe
                )
                return symbol, data
            except Exception as e:
                logger.error(f"Error fetching data for {symbol}: {e}")
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = [
                executor.submit(fetch_single_symbol, symbol) for symbol in symbols
            ]
