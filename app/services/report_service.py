...
class ReportService:
    ...
    def generate_pnl_distribution_chart(self, trades, symbol, strategy_name):
        ...
        try:
            ...
            plt.axvline(
                x=0, color="red", linestyle="--", linewidth=2, label="Break-even"
            )
    ...
    def generate_performance_summary_chart(self, metrics, symbol, strategy_name):
        ...
        try:
            ...
            bars = plt.bar(
                labels, values, color=colors_list, alpha=0.7, edgecolor="black"
            )
            ...
            for bar, value in zip(bars, values):
                ...
                plt.text(
                    ...
                    height
                    + (0.01 * max(values) if height >= 0 else -0.05 * min(values)),
                    f"${value:,.0f}",
                    ha="center",
    ...
    def create_pdf_report(self, metrics, symbol, strategy_name):
        ...
        try:
            ...
            title = Paragraph(
                f"Trading Strategy Backtest Report<br/>{symbol} - {strategy_name}",
                title_style,
            )
            story.append(title)
            ...
            meta_style = ParagraphStyle(
                "Meta", parent=styles["Normal"], fontSize=10, textColor=colors.gray
            )
            meta_text = (
                f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}<br/>"
            )
            meta_text += (
                f"Initial Balance: ${metrics.get('initial_balance', 100000):,}<br/>"
            )
            ...
            if trades and len(trades) > 0:
                ...
                trade_data = [
                    ["Entry Time", "Entry Price", "Exit Time", "Exit Price", "P&L"]
                ]
    ...
    def generate_strategy_comparison_report(self, comparison_results, symbol):
        ...
        try:
            ...
            # Comparison Table
            story.append(
                Paragraph("Strategy Performance Comparison", styles["Heading2"])
            )
            story.append(Spacer(1, 12))
