...
def strategy_3_bollinger_bands(df, period=20, std_dev=2, initial_balance=100000):
    ...
    """
    logger.info(f"Running Bollinger Bands Strategy (Period: {period}, Std Dev: {std_dev})")
    ...
    df['band_position'] = ((df['c'] - df['lower_band']) / (df['upper_band'] - df['lower_band'])) * 100
    ...
    for i in range(period, len(df)):
        ...
        if position is None and current_price <= lower_band * 1.001:  # Allow 0.1% tolerance
            position = 'BUY'
            entry_price = current_price
            entry_time = current_time
            logger.debug(f"BUY signal at {current_time}: Price={current_price:.2f}, Lower Band={lower_band:.2f}")

        # Check for sell signal (price near or above upper band)
        elif position == 'BUY' and current_price >= upper_band * 0.999:  # Allow 0.1% tolerance
            ...
            trade = {
                ...
                'duration': duration,
                'type': 'BUY',
                'entry_band_pos': float(df.iloc[max(0, i - duration)]['band_position']) if duration < i else 0,
            ...
            position = None
            logger.debug(f"SELL signal at {exit_time}: Price={exit_price:.2f}, Upper Band={upper_band:.2f}, P&L: {pnl}")
    ...
    if position == 'BUY':
        ...
        trade = {
            ...
            'duration': duration,
            'type': 'BUY',
            'entry_band_pos': float(df.iloc[max(0, len(df) - duration - 1)]['band_position']),
            'exit_band_pos': float(exit_band_pos)
        }
        trades.append(trade)
        trade_durations.append(duration)

        logger.debug(f"Closed open position at end: Price={exit_price:.2f}, Band Pos={exit_band_pos:.2f}, P&L: {pnl}")
