...
def strategy_4_macd_crossover(
    df, fast_period=12, slow_period=26, signal_period=9, initial_balance=100000
):
    """
    ...
    """
    logger.info(
        f"Running MACD Strategy (Fast: {fast_period}, Slow: {slow_period}, Signal: {signal_period})"
    )
    ...
    for i in range(start_idx, len(df)):
        ...
        if (
            position is None
            and df.iloc[i]["macd_above_signal"]
            and not df.iloc[i]["prev_macd_above_signal"]
        ):
            position = "BUY"
            entry_price = current_price
            entry_time = current_time
            logger.debug(
                f"BUY signal at {current_time}: MACD={macd_line:.4f}, Signal={signal_line:.4f}"
            )

        # Check for sell signal (MACD crosses below signal)
        elif (
            position == "BUY"
            and not df.iloc[i]["macd_above_signal"]
            and df.iloc[i]["prev_macd_above_signal"]
        ):
            ...
            trade = {
                ...
                "type": "BUY",
                "entry_macd": (
                    float(df.iloc[max(0, i - duration)]["macd_line"])
                    if duration < i
                    else 0
                ),
                "exit_macd": float(macd_line),
                "entry_signal": (
                    float(df.iloc[max(0, i - duration)]["signal_line"])
                    if duration < i
                    else 0
                ),
            ...
            position = None
            logger.debug(
                f"SELL signal at {exit_time}: MACD={macd_line:.4f}, Signal={signal_line:.4f}, P&L: {pnl}"
            )
    ...
    if position == "BUY":
        ...
        trade = {
            ...
            "entry_signal": float(
                df.iloc[max(0, len(df) - duration - 1)]["signal_line"]
            ),
        ...
        logger.debug(
            f"Closed open position at end: MACD={exit_macd:.4f}, Signal={exit_signal:.4f}, P&L: {pnl}"
        )
