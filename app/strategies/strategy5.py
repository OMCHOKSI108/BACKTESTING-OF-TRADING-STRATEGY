...
def strategy_5_multi_indicator(df, rsi_period=14, ema_short=9, ema_long=21, rsi_overbought=70, rsi_oversold=30, initial_balance=100000):
    """
    ...
    """
    logger.info(f"Running Multi-Indicator Strategy (RSI: {rsi_period}, EMA: {ema_short}/{ema_long})")
    ...
    for i in range(start_idx, len(df)):
        ...
        if (position is None and
            ...
            df.iloc[i]['ema_short_above_long'] and
            not df.iloc[i]['prev_ema_short_above_long']):

            position = 'BUY'
            entry_price = current_price
            entry_time = current_time
            logger.debug(f"BUY signal at {current_time}: RSI={current_rsi:.2f}, EMA crossover")

        # Check for sell signal (RSI overbought OR EMA bearish crossover)
        elif (position == 'BUY' and
              (current_rsi > rsi_overbought or
               (not df.iloc[i]['ema_short_above_long'] and df.iloc[i]['prev_ema_short_above_long']))):
            ...
            exit_reason = 'RSI_overbought' if current_rsi > rsi_overbought else 'EMA_crossover'
            ...
            trade = {
                ...
                'duration': duration,
                'type': 'BUY',
                'entry_rsi': float(df.iloc[max(0, i - duration)]['rsi']) if duration < i else current_rsi,
            ...
            position = None
            logger.debug(f"SELL signal at {exit_time}: {exit_reason}, RSI={current_rsi:.2f}, P&L: {pnl}")
